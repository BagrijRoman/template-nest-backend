...
- Single-use tokens mailed to a user (password reset, email verification, and whatever comes next — an invite, a magic link) all live in `actiontokens` through `ActionTokensService` (`src/auth/actionTokens.service.ts`): one row per user and `ActionTokenType`, stored as a sha256 hash, redeemed by an atomic find-and-delete that matches the action as well as the hash, so a token issued for one flow can never be redeemed as another. A new mailed-token flow adds an enum value and a TTL constant, never a collection; the flow service keeps only its own rate limiting, mail and follow-up.
- Device sessions: a `sessions` document (`src/auth/entities/session.entity.ts`) is what a person recognizes as a signed-in device, and refresh tokens rotate inside it (`sessionId`). Sign-up and sign-in open one and record the User-Agent and address through the `@Client()` decorator; rotation keeps it alive for exactly as long as the newest refresh token. Anything that belongs to a device rather than an account (a push notification token, an analytics id) goes on this document, not on `User`. `RefreshTokensService` owns revocation and always ends tokens and session row together — a session with live tokens, or a token without a session, is a bug. The access token names its session (`sid`), which `JwtAuthGuard` attaches and `@CurrentSessionId()` reads. Revoking one session kills its refresh token at once, while that device's access token still works until it expires (minutes); the immediate, account-wide answer is the password change.
...
- Dependency hygiene: `npm audit --audit-level=high` gates every CI run (`.github/workflows/ci.yml`) and release; the lockfile is always committed. High-severity findings in runtime dependencies block the release. `package.json` carries no `overrides`: an override is a temporary patch around an upstream lag, so whenever one is added, note what would let it go and remove it as soon as that lands.
...
- Rate limiting is global via `@nestjs/throttler` (`ThrottlerGuard` as `APP_GUARD`) with two per-IP windows (`src/common/constants.ts`): a sustained per-minute limit plus a short burst window that cuts request floods off within a second; `/auth/*` carries a stricter profile (`auth.constants.ts`), `@SkipThrottle()` on `/health-check` (probes must never be limited); never remove the guard. App-level throttling only blunts basic floods — real DDoS protection belongs upstream (CDN/WAF). Behind a reverse proxy set `TRUST_PROXY` (hop count or proxy whitelist, validated at startup and applied in `setupApp`) so limits key on the client IP, not the proxy's — never `true`, which would let callers forge that IP through `X-Forwarded-For`.
