...
- Dependency hygiene: `npm audit` runs before every release (and in CI once it exists); the lockfile is always committed. High-severity findings in runtime dependencies block the release. Note: `package.json` `overrides` pins `multer` >=2.3.0 over `@nestjs/platform-express`'s 2.2.0 to clear DoS advisories — drop that override once platform-express catches up.
