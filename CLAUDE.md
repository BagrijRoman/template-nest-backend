...
- Roles: `UserRole` (`user` | `admin`) lives on the `User` entity (everyone signs up as `user`); `@Roles(UserRole.Admin)` (`src/common/decorators/roles.decorator.ts`) restricts a route or controller and the global `RolesGuard` (`src/common/guards/roles.guard.ts`, registered after `JwtAuthGuard`) enforces it by reading the caller's current role from the database — a role is never carried in a token, so a change applies at once. Every `@Roles()` route documents its 403 with `@ApiForbiddenResponse`. Roles are granted only by the operator CLI `npm run user:set-role -- <email> <role>` (`src/scripts/setRole.ts`); never add an endpoint that lets an account raise its own role.
