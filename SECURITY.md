...
- Passwords are hashed with **async scrypt** (libuv thread pool — the sync variant would block the event loop and act as a DoS amplifier), salted per hash, compared with `timingSafeEqual`. Hashes are stored in a separate `credentials` collection (one document per user and credential type), owned by `CredentialsService` which only ever answers booleans — the `User` entity and every user query are secret-free, so a hash cannot reach API responses or logs by accident.
...
- **Destructive account actions re-verify the password**: changing it and deleting the account both require the current password, so a stolen access token alone can neither take an account over nor destroy it; failures feed the same sign-in lockout, and deletion wipes credentials, refresh tokens and pending reset / verification tokens along with the account.
- **Revocation is immediate, not TTL-bound**: `JwtAuthGuard` loads the account on every authenticated request and refuses access tokens issued before `User.sessionsValidFrom`, which a password change or reset stamps alongside deleting the refresh tokens. A stolen access token therefore dies with the password rather than living out its TTL, and a deleted account stops authenticating at once.
- **Authorization is default-closed**: the global `JwtAuthGuard` (`src/common/guards/jwtAuth.guard.ts`) requires a valid bearer access token on every route; the exceptions (`/auth/*`, `/health-check`, `/`) are explicitly marked `@Public()`, so a forgotten decorator fails closed, never open. Missing, malformed, expired and forged tokens are answered with the same generic 401. The guard verifies by signature alone (no database hit per request); `GET /users/me` re-checks account existence and forces re-auth when the account is gone. The guard runs after the throttler, so unauthenticated floods burn the rate limit first.
- **Roles are checked against the database, not the token**: `@Roles()` routes go through `RolesGuard`, which loads the caller's current role, so revoking admin takes effect immediately instead of when the access token expires. There is no endpoint that grants a role — only the operator CLI `npm run user:set-role`; the change is logged as a security event.
- **Device sessions**: every sign-in opens a `sessions` row that refresh tokens rotate inside, so a person can see where they are signed in (`GET /auth/sessions`) and end one device (`DELETE /auth/sessions/:id`) without disturbing the others. Revoking a session deletes its refresh tokens with it; the User-Agent and address stored on it are shown back to the owner only, and the address is only as trustworthy as `TRUST_PROXY` makes it. A session id that is not the caller's gets a 404, so the endpoint confirms nothing about other accounts.
- **Refresh-token reuse detection with session revocation**: every sign-in/sign-up starts a device session (`sessionId` in the store) and rotation stays inside it. Consumed records are kept until their TTL instead of deleted, so a replayed token is distinguishable from a forged one — and a consumed token showing up again is proof of theft (thief or victim holds a copy that already rotated): the whole session is revoked, while the user's other device sessions stay untouched. The stance is strict — a legitimate concurrent double-refresh also ends that session rather than risk leaving a stolen token alive. The caller still sees only the generic 401; the event is logged server-side as a warning.
...
- **Single-use tokens are scoped to their action**: reset and verification tokens share one store, and redemption matches the action alongside the hash, so a token mailed for one purpose cannot be spent on another. Only hashes are stored, each token works exactly once (atomic find-and-delete) and expiry is checked in code as well as by the TTL index.
...
2. `consume`: atomically mark the record consumed by hash (`findOneAndUpdate`). No unconsumed record → either the token was never real (nothing else happens) or it _was_ consumed before — reuse, and the whole session is revoked. Both answer the same generic 401.
